...
export default {
  async getProfile(req: Request, resp: Response): Promise<Response> {
    try {
      ...
      const userProfile = await userService.getUsers();
  ...
  async deleteProfile(req: Request, resp: Response): Promise<Response> {
    try {
      const userService = new UserService(new UserRepository());
      const userCpf = req.headers["userCpf"] as string;
      const userProfile = await userService.deleteUser(userCpf);
  ...
  //   try {
  //     const { name, id, email, cpf,ra } = req.body as UserDTO;
  ...
  //     const userService = new UserService(new UserRepository());
  //     await userService.postCreateUser({ name, email, cpf, ra });
